...
class CustomDriver(webdriver.Firefox,
    ...
    def __init__(self, ARGUMENTS):
        ...
        if ARGUMENTS.D == 'Firefox':
            ...
            webdriver.Firefox.__init__(
                self, executable_path=path_binary, options=options)
            ...
            webdriver.Chrome.__init__(
                self, executable_path=path_binary, chrome_options=options)
            ...
            if platform == "win32":
                ...
                raise Exception("Edge is only supported on Windows ! ")
        else:
            print(
                'Unsupported driver, please use only "Firefox", "Opera", "Edge" or "Chrome".')
            raise Exception('BROWSER NOT SUPPORTED !')

        self.DiscordLogin(self.CHANNEL)
        self.SEND_MSG_BAR = self.FindOneByXPATH(
            self.SEND_MSG_BAR_XPATH, 'Discord sending bar')
        self.LAST_MSG = self.FindMessages('find last message')[0]

    def DiscordLogin(self, CHANNEL: str):
        # if the browser hasn't loaded the page the css elements cannot be found
        self.get(CHANNEL)
        sleep(3)
        ...
        if self.TOKENFA:
            input(
                f"Please type this code : '{ TOTP(self.TOKENFA).now() }' and then come back to this console to type enter.")
    ...
    def FindOneByCLASS(self, _class: str, item: str,
        ...
        try:
            ...
            raise Exception(
                f'"{ item }" took too much time to apear, dev can\'t really do anything to fix it')
    ...
    def WaitNew(self, to_send: str, contains,  # contains can be a str or a list of str
        ...
        if not message_orig_txt:
            message_orig_txt = self.LAST_MSG.text
        # send le message
        ...
        if new_message and message_orig_txt != new_message.text:
            return new_message
    ...
    def attente(self, time: int, arg: str):
        for remaining in range(time, 0, -1):
            print(
                f"{remaining:2d}/{ time } seconds remaining before { arg }.", end='\r')
            sleep(1)
        if platform != 'win32':
            stdout.write("\033[K")
            stdout.flush()
    ...
    def Vote(self):
        ...
        self.FindOneByXPATH(self.AUTHORIZE_BUTTON_XPATH,
                            'Discord Authorize button on top.gg').click()
        self.FindOneByCLASS('logo', 'Top.gg main logo')
        self.get(self.URL_VOTE_POKEMEOW)
        try:
            sleep(5)
            self.FindOneByXPATH(
                "//a[@class='btn' and text() = 'No thanks']", 'No tanks button', 15).click()
        except:
            pass
        sleep(5)
